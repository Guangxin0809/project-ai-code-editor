...
export const processmessage = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const codingAgent = createAgent({
      ...
      //   defaultParameters: { temperature: 0.3, max_tokens: 16_000 },
      // }),
      tools: [
    ...
    const network = createNetwork({
      ...
      router: ({ network }) => {
        ...
        // Anthropic outputs text & tool calls together
    ...
    // Update the assiatant message with the response (this also sets status to completed)
