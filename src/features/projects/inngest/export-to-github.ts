...
export const exportToGithub = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const files = await step.run("fetch-project-files", async () => {
      return (
        await convex.query(
          api.system.getProjectFilesWithUrls,
          {
            internalKey,
            projectId,
          },
    ...
    // Filter to pnly actual files (no folders)
    ...
    const treeItems = await step.run("create-blobs", async () => {
      ...
      for (const [path, file] of fileEntries) {
        ...
        if (file.content !== undefined) {
          // Text file ("content" property is just for text file)
          content = file.content
        } else if (file.storageUrl) {
          // Binary file - fetch and base64 encode
          const response = await ky.post(file.storageUrl);
