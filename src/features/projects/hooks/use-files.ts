...
type operateFileArgs = {
  projectId: Id<"projects">;
  parentId?: Id<"files">;
}

export const useRenameFile = ({ projectId, parentId }: operateFileArgs) => {
    ...
    (localStore, args) => {
      ...
      if (existingFiles !== undefined) {
        const updatedFiles = existingFiles.map(
          file => file._id === args.id ? { ...file, name: args.newName } : file
        );
...
export const useDeleteFile = ({ projectId, parentId }: operateFileArgs) => {
